...
this.options.point_type&&f.fillRect(g[c]-m,h[c]-m,i,i);if(!0==this.options.trails){if(e=l.values[this.MAX_UNITS*c+b-1])e=0==e?0:Math.floor((j.length-1)*(Math.log(e)/this.MAX_VALUE_LOG)),e!=a&&(a=e<j.length?e:a,f.fillStyle=j[a]),"circle"==this.options.point_type?f.drawImage(d.sprite_2[a],g[c]-o-1,h[c]-o-1):"square"==this.options.point_type&&f.fillRect(g[c]-m,h[c]-m,o,o);if(!IOS&&(e=l.values[this.MAX_UNITS*c+b-2]))e=0==e?0:Math.floor((j.length-1)*(Math.log(e)/this.MAX_VALUE_LOG)),e!=a&&(a=e<j.length?
e:a,f.fillStyle=j[a]),"circle"==this.options.point_type?f.drawImage(d.sprite_3[a],g[c]-i,h[c]-i):"square"==this.options.point_type&&f.fillRect(g[c]-m,h[c]-m,i,i)}}}};String.prototype.format=function(a,b,c){return function(){var d=this,g=arguments.length+1;for(a=0;a<g;c=arguments[a++])b="object"===typeof c?JSON.stringify(c):c,d=d.replace(RegExp("\\{"+(a-1)+"\\}","g"),b);return d}}();function Profiler(){}Profiler.times={};
...
Profiler.get=function(a){return{t0:null,start:function(){this.t0=(new Date).getTime()},end:function(){null!==this.t0&&(Profiler.new_time(a,this.time=(new Date).getTime()-this.t0),this.t0=null)}}};void 0==Function.prototype.bind&&(Function.prototype.bind=function(a){var b=this;return function(){var c=Array.prototype.slice.call(arguments);return b.apply(a||null,c)}});function Torque(){var a=Array.prototype.slice.call(arguments),b=a.pop(),a=a[0]&&typeof a[0]==="string"?a:a[0],c;if(!(this instanceof Torque))return new Torque(a,b);if(!a||a==="*"){a=[];for(c in Torque.modules)Torque.modules.hasOwnProperty(c)&&a.push(c)}for(c=0;c<a.length;c=c+1)Torque.modules[a[c]](this);b(this);return this}
Torque.modules={};Torque.modules.app=function(a){a.app={};a.app.Instance=Class.extend({init:function(b){this.layers={};a.log.enabled=b?b:false},addLayer:function(b,c){return new a.layer.Engine(b,c)}})};
Torque.modules.layer=function(a){a.layer={};a.layer.Engine=Class.extend({init:function(b,c){this._defaults={user:"viz2",table:"ny_bus",column:"timestamp",steps:250,resolution:3,cumulative:false,fps:24,autoplay:true,clock:false,zindex:0,fitbounds:false,countby:"count(i.cartodb_id)",blendmode:"source-over",trails:false,point_type:"square"};this.options=_.defaults(c,this._defaults);this._map=b;for(this._index=this.options.zindex;this._map.overlayMapTypes.length<this.options.zindex;)this._map.overlayMapTypes.push(null);
this._cartodb=new Backbone.CartoDB({user:this.options.user});this.bounds=new google.maps.LatLngBounds;a.clock.enabled=this.options.clock?this.options.clock:false;a.clock.set('<span class="loading">loading...</span>');this.getDeltas()},pause:function(){if(this.running==true)this.running=false;else{this.running=true;this.play()}},setOptions:function(b){this.running=false;this.options=_.defaults(b,this._defaults);a.clock.enabled=this.options.clock?this.options.clock:false;a.clock.set('<span class="loading">loading...</span>');
...
this._display);this.fitBounds(this.options.fitbounds);this.running=false;a.clock.clear();if(this.options.autoplay){this.running=true;this.play()}a.log.info("Layer is now running!")},_setupListeners:function(){var a=this;google.maps.event.addListener(this._map,"zoom_changed",function(){a._display.reset_max_value()})},getBounds:function(){return this.bounds},fitBounds:function(a){if(a!==false){this._map.fitBounds(this.bounds);typeof a=="number"?this._map.setZoom(this._map.getZoom()+a):this._map.setZoom(this._map.getZoom())}},
...
function(){this.from<b&&this.to>b&&a.subtitles._update(this.sub)})};a.subtitles._update=function(a){$(".torque_subs").html(a)}};Torque.modules.log=function(a){a.log={};a.log.info=function(b){a.log._torquete("INFO: "+b)};a.log.warn=function(b){a.log._torquete("WARN: "+b)};a.log.error=function(b){a.log._torquete("ERROR: "+b)};a.log.todo=function(b){a.log._torquete("TODO: "+b)};a.log._torquete=function(b){var c=window.console;if(a.log.enabled){c&&c.markTimeline&&c.markTimeline(b);console.log(b)}}};
